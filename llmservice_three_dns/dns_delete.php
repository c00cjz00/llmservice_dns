<?php
# example: php dns_delete.php $clouflarekey _nctu.biobank.org.tw 
...
for ($i=0;$i<count($arr);$i++){
 ...
 if (substr($DNS_name,-$len)==$DNS_KEY){
  echo $DNS_name."\n";
  $cmd='curl -X DELETE "https://api.cloudflare.com/client/v4/zones/'.$ZONE_ID.'/dns_records/'.$DNS_id.'" -H "Authorization: Bearer '.$ACCESS_TOKEN.'" -H "Content-Type: application/json"';
  echo $cmd."\n";
  echo shell_exec($cmd)."\n";
 }
} 
