...
class StopWordsFilter(Filter):
    ...
    def __init__(self,
        ...
        self.model_key = None
        self.lang = lang
        self.STOPWORDS = json.load(open('assets/stopwords_cn_char.json', 'r'))
    ...
    def compute_stats(self, sample, context=False):
        ...
        for stopword in self.STOPWORDS['all']:
            if stopword in sample[self.text_key]:
                sample[Fields.stats]['stopwords_cnt'] += 1
                if sample[Fields.stats]['stopwords_cnt'] >= self.min_cnt:
                    break
